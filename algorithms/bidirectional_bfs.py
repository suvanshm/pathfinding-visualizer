...
def reconstruct_path(start_previous, end_previous, intersect_node, win, grid50):
    """
    Reconstructs the shortest path from start node to end node through the intersection node.

    Args:
    - start_previous: dictionary containing the previous node for each node in the path from start node to intersection node
    - end_previous: dictionary containing the previous node for each node in the path from intersection node to end node
    - intersect_node: the intersection node where the two BFS searches meet
    - win: the Pygame window object
    - grid50: the Grid object representing the grid

    Returns:
    - The length of the reconstructed path
    """
    len_path = 0
    ...
    while current is not None:
        len_path += 1
        current.path = True # changing color of spot is linked to different attributes so changing them automatically generates the animation
        grid50.draw_grid(win)
        pygame.display.update()
        current = start_previous.get(current) # moving backwards from intersection node to start node
